...
- **Pan / Tilt from block orientation**: the speaker blocks in this scene physically point along **local −Y**, so `FORWARD_LOCAL = (0, -1, 0)`. After transforming the vector by `InstanceXform`, we explicitly `Reverse()` it (using the back of the block as the reference axis) so the downstream `pan_tilt` formula stays a simple `-atan2(y, x)` for pan (the single negation that matches Holophonix's sign convention) and `atan2(z, horizontal)` for tilt. If the front face of a block points along another local axis, change `FORWARD_LOCAL`.
